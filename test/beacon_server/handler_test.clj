...
(deftest game
  (let [matches (atom {"match-1" (assoc match-with-one-player :status :started)})]
    ...
    (let [{:keys [status body]} (request-with-matches :get "/api/match/match-1/status/player-1" matches)]
      (is (= 200 status))
      (is (= {:current-round 0 :next-player-name "player-1-name"}
             (select-keys body [:next-player-name :current-round]))))))
