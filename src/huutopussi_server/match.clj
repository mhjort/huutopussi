...
(defn start-match-loop [matches teams id players starting-players]
  (let [poison-pill (chan)
        player-wait-chan (chan)
        reset-player-starts #(map-vals (fn [player-stats]
                                         (assoc player-stats :ready-to-start? false))
                                       %)]
    (go-loop [starting-players-in-order starting-players]
             (swap! matches #(update % id assoc :match-loop-poison-pill poison-pill))
             (swap! matches #(update % id reset-player-starts))
             (swap! matches #(update % id assoc :all-players-ready player-wait-chan))
             ;TODO Wait also poison pill
             (<! player-wait-chan)
